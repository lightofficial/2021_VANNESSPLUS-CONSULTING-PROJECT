...
<body>
    ...
    if (isset($_POST['cId'])) {
        ...
        $sql = "SELECT * FROM usertable WHERE email='$emailAdmin'";
        $res = mysqli_query($con, $sql);
        $row2 = mysqli_fetch_assoc($res);
        $select = "SELECT * FROM candidate LEFT JOIN candidateresume ON candidate.tId=candidateresume.tId WHERE candidateId='$candidateId'";
    ...
    if ($pdf == null) { ?>
        ...
        <div class="wrapper">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-6 offset-xl-3 col-lg-6 offset-lg-3 col-md-12 full-dark-bg">
                        ...
                        <form action="file-upload.php" method="POST" class="dropzone files-container" enctype="multipart/form-data">
                            <input type="hidden" name="canId" id="canId" value=" <?php echo $candidateId ?>">
                            <input type="hidden" name="admin" id="admin" value=" <?php echo $emailAdmin ?>">
                            <div class="fallback">
            ...
            <form action="views/action/success.php" method="POST">
            <div class="op" style="float: right;padding-right:315px">
            <input type="hidden" name="canId123" id="canId123" value=" <?php echo $candidateId ?>">
            <input type="hidden" name="admin123" id="admin123" value=" <?php echo $emailAdmin ?>">
            <?php if($row2['userStatus']==1){ ?>
                <button type="submit" id="successAdmin" name="successAdmin" class="btn btn-success" >Submit</button></form>
            <?php } else {?>
                <button type="submit" id="success" name="success" class="btn btn-success" >Submit</button></form>
            <?php } ?>
        </div></div>
    ...
    <?php } else { ?>
        ...
        <iframe src="<?php echo $secret . $pdf; ?>" width="100%" height="100%">
        </iframe>
