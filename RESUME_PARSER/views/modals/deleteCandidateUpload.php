...
if (isset($_POST["canIdUpload"])) {
    $xx = $_POST["canIdUpload"];
    $email =$_POST["email"];
    $output = '';
    $con = mysqli_connect('localhost', 'root', '', 'super');
    $sql1 = "SELECT * FROM parser_resume_data WHERE id='" . $_POST["canIdUpload"] . "'";
    $res1 = mysqli_query($con, $sql1);
    $row1 = mysqli_fetch_array($res1);
    $output .= '<div class="modal-header">
                    <h4 class="modal-title">Delete ' . $row1['data_name_first'] . '</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                </div>  
                <form action="views/action/deleteCandidate2.php" method="POST">
